...
// are exposed separately for self-service flows driven from token-continent's own UI.
...
func WaffoPancakeBuyerIdentityFromUserID(userID int) string {
	return fmt.Sprintf("token-continent-user-%d", userID)
}
...
const (
	defaultWaffoPancakeStoreName   = "token-continent-store"
	defaultWaffoPancakeProductName = "token-continent-charge-product"
)
...
//
// OnetimeProduct (not SubscriptionProduct) because token-continent has no renewal-
// event handling; Pancake auto-renewing without token-continent extending user
